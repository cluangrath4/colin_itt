...
thread_local std::stringstream g_trace_buffer; // buffer for this thread
thread_local bool g_needs_comma = false; // for the comma
...
// write the buffer to the real file
void flush_thread_buffer() {
    std::string buffer_content = g_trace_buffer.str();
    if (!buffer_content.empty()) {
        ...
        g_trace_buffer.str(""); // Clear the buffer after flushing
...
void init() {
    ...
    if (f) {
        fprintf(stderr, "[Tracer] Opened trace file: %s\n", fname);
...
void cleanup() {
    // Flush any remaining data from this main thread's buffer
    flush_thread_buffer();
    ...
    if (f) {
        // lock to be safe
        std::lock_guard<std::mutex> lock(g_file_mutex);
        fprintf(stderr, "[Tracer] Closing trace file for pid %d...\n", getpid());
...
__itt_domain* __itt_domain_createA(const char* name) {
    return reinterpret_cast<__itt_domain*>(const_cast<char*>(name));
}

__itt_string_handle* __itt_string_handle_createA(const char* name) {
    return reinterpret_cast<__itt_string_handle*>(const_cast<char*>(name));
}
...
void __itt_task_begin(const __itt_domain* domain, __itt_id taskid, __itt_id parentid, __itt_string_handle* name) {
    ...
    // Push task info to this thread's stack
    ...
    // Write the 'Begin' event to the thread-local buffer
    g_trace_buffer << "{\"name\": \"" << full_name << "\", \"cat\": \"task\", \"ph\": \"B\", \"ts\": " << ts
...
void __itt_task_end(const __itt_domain* domain) {
    if (g_task_stack.empty()) {
        return; // Mismatched end call, ignore
    }

    // Get the task info from this thread's stack
    ...
    g_trace_buffer << "{\"name\": \"" << task.name << "\", \"cat\": \"task\", \"ph\": \"E\", \"ts\": " << end_time
                   << ", \"pid\": " << getpid() << ", \"tid\": " << get_my_tid() << ", \"args\": {}}";
    
    g_needs_comma = true;

    // Optional: Flush the buffer if it gets too large to avoid memory issues
    if (g_trace_buffer.tellp() > 8192) {
        flush_thread_buffer();
    }
}
